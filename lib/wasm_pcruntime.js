...
const {contract_messages, ledger_messages} = require('fabric-ledger-protos-node');
...
class Wasm_PcRuntime {
    ...
    constructor(filename, log) {
        ...
        if (!fs.existsSync(resolvedName)) {
            throw new Error(`File ${resolvedName} does not exist`);
        }
        this.source = fs.readFileSync(resolvedName);
        ...
        this.wasm = await WebAssembly.instantiate(typedArray, {
            ...
            'wapc': {
                '__guest_request': (op_ptr, param_ptr) => {
                    ...
                    console.log(this.currentFnname.toString());
                ...
                '__guest_error': (ptr, len) => {
                    ...
                    // const retval = Return.decode(this.getUint8Memory0().subarray(ptr, ptr + len), len);
                ...
                '__host_call': (bn_ptr, bn_len, ns_ptr, ns_len, op_ptr, op_len, ptr, len) => {
                    const bn = this.cachedTextDecoder.decode(this.getUint8Memory0().subarray(bn_ptr, bn_ptr + bn_len));
                    ...
                    this.log(`___host_call ns=:${ns}: op=:${op}: data=:${data}:`);
                    return 1;
                },
                '__host_response_len': () => {
